...
def recursion_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        print(f"Recursing with {args}, {kwargs}")  # 添加日志
        result = func(*args, **kwargs)
        print(f"Result: {result}")  # 添加日志
        return result
    return wrapper
...
# 测试
test_structure = [1, 2.0, "string", True, [1, 2, [3, 4]], {"a": 1, "b": [1, 2, 3]}]
generate_random_structure(test_structure)
