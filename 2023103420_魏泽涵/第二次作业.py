...
class DataGenerator:
    ...
    def generate_data(self, count, datetype, operation=None, **kwargs):
        """
        生成指定类型和数量的数据，并可选择进行某种统计操作。

        :param count: 生成数据的数量
        :param datetype: 生成数据的类型，支持'string', 'list', 'dict', 'tuple'
        :param operation: 统计操作，支持'mean', 'sum'
        :param kwargs: 生成数据的模板
        :return: 生成数据或统计数据的结果
        """
...
# 生成多个元组并计算所有数字节点的均值
result_tuples, mean_value_tuples = generator.generate_data(3, 'tuple', operation='mean', x=5, y=10, z=15)
print("\n生成的元组:", result_tuples)
print("数字节点的均值:", mean_value_tuples)

# 生成一个字符串并计算所有数字节点的总和
result_string, total_sum_string = generator.generate_data(1, 'string', operation='sum', a=100, b=200, c=300)
print("\n生成的字符串:", result_string)
print("数字节点的总和:", total_sum_string)
