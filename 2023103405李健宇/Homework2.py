...
class RandomDataGenerator:
    ...
    def generate_structure(struct):
        if isinstance(struct, dict):
            return {k: RandomDataGenerator.generate_structure(v) for k, v in struct.items()}
        elif isinstance(struct, list):
            return [RandomDataGenerator.generate_structure(struct[0]) for _ in range(len(struct))]
        elif isinstance(struct, tuple):
            return tuple(RandomDataGenerator.generate_structure(struct[0]) for _ in range(len(struct)))
        else:
            return RandomDataGenerator.generate_value(struct)

    @staticmethod
    def generate_random_data(structure, size=1):
        return [RandomDataGenerator.generate_structure(structure) for _ in range(size)]
