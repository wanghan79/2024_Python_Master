...
def count_info_generation(func):
    def wrapper(*args, **kwargs):
        ...
        print(f"Call {wrapper.count} times to {func.__name__}, generated {len(result)} user information records.")
        return result
    wrapper.count = 0
    return wrapper
...
class UserInfoGenerator:
    ...
    def save_to_csv(self, user_infos):
        try:
            ...
            messagebox.showerror("Save Error", f"Error occurred while saving user information: {e}")
...
class UserInfoGeneratorGUI:
    ...
    def generate_users(self):
        ...
        try:
            user_requirements = ast.literal_eval(user_input.strip())
            self.user_infos = self.generator.generate_info(user_requirements)
            messagebox.showinfo("Success", f"Successfully generated user information.")
            # Optionally clear input after generating
    ...
    def save_users(self):
        try:
            ...
            messagebox.showinfo("Success", f"Successfully saved user information.")
        except Exception as e:
